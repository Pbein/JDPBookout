...
class JDPowerApp:
    ...
    def create_widgets(self):
        ...
        # Activity log removed for cleaner interface
    ...
    def load_saved_settings(self):
        ...
        # Username field starts blank - no auto-fill
        # This prevents showing previously used usernames
        
        # Try to load saved password only if credentials were saved
        if self.prefs.get("save_credentials", False):
            # Don't auto-fill username, but can still load password if user enters the same username
            pass
    ...
    def start_download(self):
        ...
        # Clear log (activity log removed)
    ...
    def stop_download(self):
        ...
        if self.worker:
            ...
            self.status_label.config(text="Stopping... (will finish current task)")
    ...
    def handle_result(self, result):
        ...
        if result_type == 'start':
            ...
            if self.total_items > 0:
                self.progress_text.config(text=format_progress(0, self.total_items))
            ...
            # Debug message - show in status
            debug_msg = result.get('message', '')
            self.status_label.config(text=f"Debug: {debug_msg}")
        
        elif result_type == 'stop_requested':
            # Stopping...
            self.status_label.config(text="Stop requested - finishing current task...")
    ...
    # Log method removed - no longer using activity log
